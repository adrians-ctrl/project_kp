...
@section('content')
    ...
    <x-ui.page-header
        title="Edit Berita"
        desc="Perbarui artikel: {{ $berita->judul }}"
        :back="route('admin.berita.index')"
    />

    <form method="POST" action="{{ route('admin.berita.update', $berita) }}"
        ...
        <div class="grid gap-6 lg:grid-cols-3">
            ...
            <div class="order-1 lg:order-2 space-y-6">
                ...
                <x-ui.section-card title="Pengaturan Publikasi">
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center justify-between rounded-lg border
                                    border-[var(--border)] px-4 py-3"
                             x-data="{ pub: {{ old('is_published', $berita->is_published) ? 'true' : 'false' }} }">
                            <div>
                        ...
                        <div class="rounded-lg bg-[var(--muted)]/50 px-3 py-2 text-xs text-[var(--muted-foreground)]">
                            <span class="font-medium">Slug:</span> {{ $berita->slug }}
                        </div>
                    </div>
                </x-ui.section-card>
                ...
                <x-ui.section-card title="Gambar Utama">
                    <div class="space-y-3">
                        <div class="aspect-video w-full overflow-hidden rounded-lg border border-[var(--border)]
                                    bg-[var(--muted)] flex items-center justify-center">
                            <img :src="preview ?? '{{ $berita->gambar_url ?? '' }}'"
                        ...
                        <div class="space-y-1.5">
                            ...
                            <textarea name="konten" rows="16" required
                                      class="w-full rounded-md border border-[var(--input)] bg-[var(--card)]
                                             ...
                                             focus:border-[var(--ring)] focus:ring-2 focus:ring-[var(--ring)]/20
                                             {{ $errors->has('konten') ? 'border-red-400 bg-red-50' : '' }}">{{ old('konten', $berita->konten) }}</textarea>
                            @error('konten')
                                <p class="text-xs text-[var(--destructive)]">{{ $message }}</p>
                            @enderror
                        </div>
