...
@section('breadcrumb')
    ...
    <span class="font-medium text-[var(--foreground)]">Rekap Absensi</span>
@endsection
...
@section('content')
    ...
    <x-ui.page-header
        title="Rekap Absensi"
        desc="Rekap kehadiran siswa per bulan dengan persentase dan ringkasan statistik."
    >
        <x-slot:actions>
            <div x-data="{ open: false }" class="relative">
                ...
                <div x-show="open" @click.outside="open = false"
                     ...
                     style="display:none">
                    <a href="{{ route('admin.rekap.absensi.export.pdf', request()->only(['kelas_id','bulan','tahun'])) }}"
                    ...
                    <a href="{{ route('admin.rekap.absensi.export.excel', request()->only(['kelas_id','bulan','tahun'])) }}"
                       class="flex items-center gap-2.5 px-4 py-2 text-sm text-[var(--foreground)] hover:bg-[var(--muted)] transition-colors">
        ...
        <form method="GET" action="{{ route('admin.rekap.absensi') }}">
            <div class="flex flex-col gap-3 sm:flex-row sm:flex-wrap sm:items-end">
                ...
                <div class="flex gap-2">
                    ...
                    <x-ui.button variant="secondary" href="{{ route('admin.rekap.absensi') }}">Reset</x-ui.button>
                </div>
            </div>
        </form>
    </x-ui.section-card>
    ...
    @if ($rekap->count() > 0)
        <div class="grid grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6">
            @foreach ([
                ['label'=>'Total Siswa',  'value'=>$stats['total_siswa'],     'color'=>'text-[var(--foreground)]',  'bg'=>'bg-[var(--card)]'],
                ['label'=>'Total Hadir',  'value'=>$stats['total_hadir'],     'color'=>'text-green-600',  'bg'=>'bg-green-50'],
                ['label'=>'Total Izin',   'value'=>$stats['total_izin'],      'color'=>'text-yellow-600', 'bg'=>'bg-yellow-50'],
                ['label'=>'Total Sakit',  'value'=>$stats['total_sakit'],     'color'=>'text-blue-600',   'bg'=>'bg-blue-50'],
                ['label'=>'Total Alpha',  'value'=>$stats['total_alpha'],     'color'=>'text-red-600',    'bg'=>'bg-red-50'],
                ['label'=>'Rata % Hadir', 'value'=>$stats['rata_persentase'].'%', 'color'=>'text-[var(--primary)]', 'bg'=>'bg-[var(--primary-soft)]'],
            ] as $card)
                <div class="rounded-xl border border-[var(--border)] {{ $card['bg'] }} p-4 text-center">
                    <div class="font-display text-2xl font-bold {{ $card['color'] }} tabular-nums">
                        {{ $card['value'] }}
                    </div>
                    <div class="mt-1 text-[10px] font-medium text-[var(--muted-foreground)] uppercase tracking-wide">
                        {{ $card['label'] }}
                    </div>
                </div>
            @endforeach
    ...
    >
        ...
        @if ($rekap->isEmpty())
            <x-ui.empty-state
                ...
                message="Belum ada data absensi untuk periode dan filter yang dipilih."
            />
        @else
            <div class="overflow-x-auto">
                <table class="w-full text-sm">
                    ...
                    <tfoot class="border-t-2 border-[var(--border)] bg-[var(--muted)]/50">
                        <tr>
                            <td colspan="3" class="px-5 py-3 text-sm font-semibold text-[var(--foreground)]">Total</td>
                            <td class="px-5 py-3 text-center font-bold tabular-nums text-green-600">{{ $stats['total_hadir'] }}</td>
                            <td class="px-5 py-3 text-center font-bold tabular-nums text-yellow-600">{{ $stats['total_izin'] }}</td>
                            <td class="px-5 py-3 text-center font-bold tabular-nums text-blue-600">{{ $stats['total_sakit'] }}</td>
                            <td class="px-5 py-3 text-center font-bold tabular-nums text-red-600">{{ $stats['total_alpha'] }}</td>
                            <td class="px-5 py-3 text-center text-sm font-semibold text-[var(--primary)]">
                                {{ $stats['rata_persentase'] }}%
                            </td>
                        </tr>
                    </tfoot>
                </table>
