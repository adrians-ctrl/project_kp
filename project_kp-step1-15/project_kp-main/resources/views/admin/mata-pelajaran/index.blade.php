...
<div x-data="{ modalTambah: false }">
    ...
    <x-ui.page-header title="Mata Pelajaran" desc="Manajemen daftar mata pelajaran yang tersedia di sekolah.">
        <x-slot:actions>
    ...
    <x-ui.section-card :noPadding="true">
        <x-slot:actions>
            <span class="text-xs text-[var(--muted-foreground)]">{{ $mapel->total() }} mata pelajaran</span>
        </x-slot:actions>

        @if ($mapel->isEmpty())
            <x-ui.empty-state title="Belum ada mata pelajaran" message="Tambahkan mata pelajaran yang diajarkan di sekolah.">
                <x-ui.button @click="modalTambah = true" size="sm">Tambah Mapel</x-ui.button>
            </x-ui.empty-state>
        @else
                    ...
                    <tbody class="divide-y divide-[var(--border)]">
                        @foreach ($mapel as $i => $item)
                            <tr class="hover:bg-[var(--muted)]/40 transition-colors" x-data="{ modalHapus: false }">
                                <td class="px-5 py-3.5 tabular-nums text-[var(--muted-foreground)]">{{ $mapel->firstItem() + $i }}</td>
                                <td class="px-5 py-3.5">
                                    <span class="font-mono text-xs font-medium rounded-md bg-[var(--muted)] px-2 py-1 text-[var(--foreground)]">
                                        {{ $item->kode_mapel }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5 font-medium text-[var(--foreground)]">{{ $item->nama_mapel }}</td>
    ...
    <div x-show="modalTambah"
        ...
        <div x-show="modalTambah"
             ...
             class="relative w-full max-w-sm rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--shadow-elevated)]">
            <div class="flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
                <h2 class="font-display text-base font-semibold text-[var(--foreground)]">Tambah Mata Pelajaran</h2>
            ...
            <form method="POST" action="{{ route('admin.mata-pelajaran.store') }}">
                ...
                <div class="space-y-4 px-5 py-5">
                    <x-ui.input name="kode_mapel" label="Kode Mata Pelajaran" placeholder="Contoh: MTK"
                        :required="true" hint="Kode singkat unik, maksimal 20 karakter." />
                    <x-ui.input name="nama_mapel" label="Nama Mata Pelajaran" placeholder="Contoh: Matematika"
                        :required="true" />
                </div>
