{{--
    ...
    Props:
      ...
      - noPadding   (bool, optional) — hilangkan padding body untuk tabel full-width

    Slot:
      - actions     — elemen di sisi kanan header
      - default     — konten body
--}}
    ...
    @if ($title || isset($actions))
        <header class="flex items-start justify-between gap-4 border-b
            ...
            <div class="space-y-0.5">
                ...
                @endif
                @if ($description)
                    <p class="text-xs text-[var(--muted-foreground)]">{{ $description }}</p>
                @endif
            </div>
