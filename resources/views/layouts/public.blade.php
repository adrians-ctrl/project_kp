...
<head>
    ...
    <title>@yield('title', $profil?->nama_sekolah ?? 'SDN Babakan 02')</title>
    <meta name="description" content="@yield('meta-description', 'Website resmi ' . ($profil?->nama_sekolah ?? 'SDN Babakan 02'))">
        ...
        <div class="mx-auto flex h-16 max-w-6xl items-center justify-between px-4 lg:px-6">
            ...
            <a href="{{ route('landing') }}" class="flex items-center gap-3 min-w-0">
                <div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-lg bg-[var(--primary)]">
                    @if ($profil?->logo)
                        <img src="{{ $profil->logo_url }}" alt="Logo" class="h-7 w-7 object-contain">
                    @else
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-[var(--primary-foreground)]"
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1 12 20.904a48.62
                                     ...
                                     .248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12 13.489a50.702 50.702
                                     0 0 1 3.74-3.342"/>
                        </svg>
                    @endif
                </div>
                ...
                @foreach ($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                       class="rounded-md px-4 py-2 text-sm font-medium transition-colors
                              {{ request()->routeIs($item['route']) || ($item['route'] === 'landing.berita' && request()->routeIs('landing.berita.detail'))
                ...
                @foreach ($navItems as $item)
                    <a href="{{ route($item['route']) }}"
                       class="rounded-md px-3 py-2 text-sm font-medium
                              {{ request()->routeIs($item['route']) ? 'bg-[var(--primary-soft)] text-[var(--primary)]' : 'text-[var(--muted-foreground)]' }}">
                        {{ $item['label'] }}
                    </a>
                @endforeach
                <a href="{{ route('login') }}"
    ...
    <footer class="border-t border-[var(--border)] bg-[var(--card)]">
        <div class="mx-auto max-w-6xl px-4 py-12 lg:px-6">
            <div class="grid gap-8 sm:grid-cols-2 lg:grid-cols-4">
                ...
                <div class="sm:col-span-2">
                    ...
                    <p class="text-sm text-[var(--muted-foreground)] leading-relaxed max-w-md">
                        {{ $profil?->alamat_lengkap ?? 'Jl. Babakan Raya No. 10, Bogor, Jawa Barat' }}
                    </p>
                    @if ($profil?->npsn)
                        <p class="mt-2 text-xs text-[var(--muted-foreground)]">NPSN: {{ $profil->npsn }}</p>
                    @endif
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-2 text-sm">
                        <li><a href="{{ route('landing') }}" class="text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors">Beranda</a></li>
                        <li><a href="{{ route('landing.berita') }}" class="text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors">Berita</a></li>
                        <li><a href="{{ route('landing.galeri') }}" class="text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors">Galeri</a></li>
                        <li><a href="{{ route('landing.kontak') }}" class="text-[var(--muted-foreground)] hover:text-[var(--primary)] transition-colors">Kontak</a></li>
                    </ul>
                </div>
                    ...
                    <ul class="space-y-2 text-sm text-[var(--muted-foreground)]">
                        ...
                        @if ($profil?->email)
                            <li class="flex items-center gap-2">
                                ...
                                <span class="truncate">{{ $profil->email }}</span>
                            </li>
                        @endif
                    </ul>
                </div>
            ...
            <div class="mt-8 border-t border-[var(--border)] pt-6 text-center text-xs text-[var(--muted-foreground)]">
                &copy; {{ date('Y') }} {{ $profil?->nama_sekolah ?? 'SDN Babakan 02' }}. Hak cipta dilindungi.
            </div>
        </div>
    </footer>
