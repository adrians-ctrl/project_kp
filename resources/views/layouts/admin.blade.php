...
<body class="h-full bg-[var(--background)] text-[var(--foreground)]" x-data="layoutAdmin()">
    ...
    <div class="flex h-full min-h-screen">
        ...
        <aside
            ...
            class="fixed inset-y-0 left-0 z-40 flex flex-col border-r
                   ...
                   text-[var(--sidebar-foreground)] transition-all duration-300
                   lg:relative lg:translate-x-0"
        >
            {{-- Brand --}}
            <div class="flex h-16 shrink-0 items-center justify-between border-b
                        border-[var(--sidebar-border)] px-4">
                <div class="flex items-center gap-2.5 overflow-hidden" x-show="sidebarOpen || !isMobile">
                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-md
                                bg-[var(--sidebar-primary)]">
                        <svg xmlns="http://www.w3.org/2000/svg"
                             class="h-5 w-5 text-[var(--sidebar-primary-foreground)]"
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M4.26 10.147a60.438 60.438 0 0 0-.491 6.347A48.62 48.62 0 0 1
                                     12 20.904a48.62 48.62 0 0 1 8.232-4.41 60.46 60.46 0 0
                                     0-.491-6.347m-15.482 0a50.636 50.636 0 0 0-2.658-.813A59.906
                                     59.906 0 0 1 12 3.493a59.903 59.903 0 0 1 10.399 5.84c-.896
                                     .248-1.783.52-2.658.814m-15.482 0A50.717 50.717 0 0 1 12
                                     13.489a50.702 50.702 0 0 1 3.74-3.342" />
                        </svg>
                    </div>
                    <div class="flex flex-col leading-tight" :class="{'hidden': !sidebarOpen && !isMobile, 'lg:flex': true}" x-show="sidebarOpen || isMobile">
                        <span class="font-display text-sm font-bold tracking-tight
                                     text-[var(--sidebar-foreground)]">SDN Babakan 02</span>
                        <span class="text-[10px] uppercase tracking-[0.14em]
                                     text-[var(--sidebar-foreground)]/60">Sistem Informasi</span>
                    </div>
                </div>

                {{-- Toggle button (desktop: collapse icon, mobile: close icon) --}}
                <button
                    @click="toggleSidebar"
                    type="button"
            ...
            <div class="shrink-0 border-t border-[var(--sidebar-border)] p-3">
                <div class="flex items-center gap-3 rounded-md px-3 py-2.5 overflow-hidden">
                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-full
                                bg-[var(--sidebar-primary)] text-sm font-semibold
                                text-[var(--sidebar-primary-foreground)]">
                        {{ strtoupper(substr(Auth::user()->name, 0, 1)) }}
                    </div>
                    <div class="min-w-0 flex-1 overflow-hidden"
                         :class="{'hidden': !sidebarOpen && !isMobile}">
                        <div class="truncate text-sm font-medium text-[var(--sidebar-foreground)]">
                            {{ Auth::user()->name }}
                        </div>
                        <div class="truncate text-[11px] text-[var(--sidebar-foreground)]/60">
                            Administrator
                        </div>
                    </div>
                </div>
                ...
                <form method="POST" action="{{ route('logout') }}">
                    ...
                    <button
                        ...
                        class="mt-1 flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm
                               text-[var(--sidebar-foreground)]/80 transition-colors
                               hover:bg-[var(--sidebar-accent)]
                               hover:text-[var(--sidebar-accent-foreground)]"
                    >
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 shrink-0"
                            ...
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M15.75 9V5.25A2.25 2.25 0 0 0 13.5 3h-6a2.25 2.25 0 0
                                     0-2.25 2.25v13.5A2.25 2.25 0 0 0 7.5 21h6a2.25 2.25 0 0
                                     0 2.25-2.25V15M12 9l-3 3m0 0 3 3m-3-3h12.75"/>
                        </svg>
        ...
        <div class="flex min-w-0 flex-1 flex-col transition-all duration-300">
            ...
            <header class="sticky top-0 z-30 flex h-16 shrink-0 items-center justify-between
                           border-b border-[var(--border)] bg-[var(--card)]/90 px-4
                ...
                <div class="ml-auto flex items-center gap-2">
                    <span class="hidden text-sm text-[var(--muted-foreground)] sm:block">
                        {{ Auth::user()->name }}
                    </span>
        ...
        function layoutAdmin() {
            return {
                ...
                handleResize() {
                    ...
                    if (!this.isMobile) {
                        // Kembalikan state terakhir dari localStorage di desktop
