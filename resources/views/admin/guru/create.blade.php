        ...
        <form method="POST" action="{{ route('admin.guru.store') }}"
                ...
                <div class="space-y-1.5">
                    ...
                    <select name="jabatan" required
                            class="w-full h-9 rounded-md border px-3 text-sm outline-none transition
                                   ...
                                   {{ $errors->has('jabatan') ? 'border-red-400' : '' }}">
            ...
            <div class="space-y-1.5">
                ...
                <div x-data="{ preview: null }" class="flex items-start gap-4">
                    <div class="h-20 w-20 shrink-0 rounded-full border-2 border-dashed border-[var(--border)]
                                flex items-center justify-center bg-[var(--muted)] overflow-hidden">
                        <img x-show="preview" :src="preview" class="h-full w-full object-cover rounded-full">
                        <svg x-show="!preview" xmlns="http://www.w3.org/2000/svg"
                             class="h-8 w-8 text-[var(--muted-foreground)]" fill="none"
                    ...
                    <div class="flex-1">
                        ...
                        <p class="mt-1.5 text-xs text-[var(--muted-foreground)]">JPG, PNG, atau WebP. Maksimal 2MB.</p>
