...
<div x-data="{ view: localStorage.getItem('guruView') || 'grid' }"
    ...
    >
        <x-slot:actions>
            {{-- Toggle view --}}
            <div class="flex items-center rounded-md border border-[var(--border)] bg-[var(--card)] p-0.5">
                <button @click="view = 'grid'" type="button"
                        :class="view === 'grid' ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'"
                        class="flex h-7 w-7 items-center justify-center rounded transition-colors" title="Tampilan kartu">
                ...
                <button @click="view = 'table'" type="button"
                        :class="view === 'table' ? 'bg-[var(--primary)] text-[var(--primary-foreground)]' : 'text-[var(--muted-foreground)] hover:text-[var(--foreground)]'"
                        class="flex h-7 w-7 items-center justify-center rounded transition-colors" title="Tampilan tabel">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
    ...
    @if ($guru->isEmpty())
        <x-ui.section-card>
            <x-ui.empty-state title="Belum ada data guru atau staf"
                message="Tambahkan data tenaga pengajar dan staf administrasi sekolah.">
                <x-ui.button href="{{ route('admin.guru.create') }}" size="sm">Tambah Guru</x-ui.button>
            </x-ui.empty-state>
        </x-ui.section-card>
    ...
    @else
                ...
                <div class="group relative flex flex-col rounded-xl border border-[var(--border)]
                    ...
                    <div class="flex items-start gap-3 mb-4">
                        <div class="relative shrink-0">
                            ...
                            @if ($item->user)
                                <span class="absolute -bottom-0.5 -right-0.5 flex h-4 w-4 items-center
                                             justify-center rounded-full bg-green-500 ring-2 ring-[var(--card)]"
                                      title="Akun terhubung: {{ $item->user->email }}">
                        ...
                        <div class="min-w-0 flex-1">
                            <h3 class="truncate text-sm font-semibold text-[var(--foreground)] leading-tight">
                                {{ $item->nama_lengkap }}
                            </h3>
                    ...
                    <div class="space-y-1.5 flex-1 mb-4">
                        ...
                        <div class="flex items-center gap-2 text-xs">
                            @if ($item->user)
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 shrink-0 text-green-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                </svg>
                                <span class="text-green-600 truncate">{{ $item->user->email }}</span>
                            @else
                                <svg xmlns="http://www.w3.org/2000/svg" class="h-3.5 w-3.5 shrink-0 text-[var(--muted-foreground)]" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                                    <path stroke-linecap="round" stroke-linejoin="round" d="M17.982 18.725A7.488 7.488 0 0 0 12 15.75a7.488 7.488 0 0 0-5.982 2.975m11.963 0a9 9 0 1 0-11.963 0m11.963 0A8.966 8.966 0 0 1 12 21a8.966 8.966 0 0 1-5.982-2.275M15 9.75a3 3 0 1 1-6 0 3 3 0 0 1 6 0Z"/>
                                </svg>
                                <span class="text-[var(--muted-foreground)]">Belum ada akun</span>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="flex items-center justify-between border-t border-[var(--border)] pt-4">
                        <x-ui.badge :tone="$item->jabatan_badge_tone">
                            {{ $item->is_guru ? 'Guru' : 'Staf' }}
                        </x-ui.badge>
        ...
        <div x-show="view === 'table'">
            <x-ui.section-card :noPadding="true">
                <x-slot:actions>
                    <span class="text-xs text-[var(--muted-foreground)]">{{ $guru->total() }} orang</span>
                </x-slot:actions>
                    ...
                    <table class="w-full text-sm">
                        <thead>
                            <tr class="border-b border-[var(--border)] bg-[var(--muted)]/50">
                                ...
                                <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-[var(--muted-foreground)]">Akun</th>
                                            ...
                                            <div class="relative shrink-0">
                                                ...
                                                @if ($item->user)
                                                    <span class="absolute -bottom-0.5 -right-0.5 flex h-3 w-3 items-center justify-center
                                                                 rounded-full bg-green-500 ring-1 ring-[var(--card)]"></span>
                                                @endif
                                            </div>
                                    ...
                                    <td class="px-5 py-3.5">
                                        @if ($item->user)
                                            <div class="flex items-center gap-1.5">
                                                ...
                                                <span class="text-xs text-[var(--muted-foreground)] truncate max-w-[140px]">
                                                    {{ $item->user->email }}
                                                </span>
                                            </div>
                                        @else
