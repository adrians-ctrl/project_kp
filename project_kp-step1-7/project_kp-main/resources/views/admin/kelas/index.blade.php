...
<div x-data="{ modalTambah: false }">
    ...
    <x-ui.page-header title="Kelola Kelas" desc="Manajemen data kelas, tingkat, tahun ajaran, dan wali kelas.">
        <x-slot:actions>
    ...
    <x-ui.section-card :noPadding="true">
        <x-slot:actions>
            <span class="text-xs text-[var(--muted-foreground)]">{{ $kelas->total() }} kelas ditemukan</span>
        </x-slot:actions>

        @if ($kelas->isEmpty())
            <x-ui.empty-state title="Belum ada data kelas" message="Mulai dengan menambahkan kelas pertama.">
                <x-ui.button @click="modalTambah = true" size="sm">Tambah Kelas</x-ui.button>
            </x-ui.empty-state>
        @else
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-[var(--border)] bg-[var(--muted)]/50">
                            ...
                            <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-[var(--muted-foreground)]">Nama Kelas</th>
                            <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-[var(--muted-foreground)]">Tingkat</th>
                            <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-[var(--muted-foreground)]">Tahun Ajaran</th>
                            <th class="px-5 py-3 text-left text-xs font-medium uppercase tracking-wide text-[var(--muted-foreground)]">Wali Kelas</th>
                            <th class="px-5 py-3 text-center text-xs font-medium uppercase tracking-wide text-[var(--muted-foreground)]">Siswa</th>
                    ...
                    <tbody class="divide-y divide-[var(--border)]">
                        @foreach ($kelas as $i => $item)
                            <tr class="hover:bg-[var(--muted)]/40 transition-colors" x-data="{ modalHapus: false }">
                                <td class="px-5 py-3.5 tabular-nums text-[var(--muted-foreground)]">{{ $kelas->firstItem() + $i }}</td>
                                <td class="px-5 py-3.5 font-medium text-[var(--foreground)]">{{ $item->nama_kelas }}</td>
                                <td class="px-5 py-3.5">
                                    <x-ui.badge tone="info">Tingkat {{ $item->tingkat }}</x-ui.badge>
                                </td>
                                <td class="px-5 py-3.5 text-[var(--muted-foreground)]">{{ $item->tahun_ajaran }}</td>
                                <td class="px-5 py-3.5 text-[var(--muted-foreground)]">{{ $item->waliKelas?->nama_lengkap ?? '—' }}</td>
                                <td class="px-5 py-3.5 text-center">
                                    <span class="inline-flex h-6 min-w-[1.5rem] items-center justify-center rounded-full
                                                  bg-[var(--primary-soft)] px-2 text-xs font-semibold text-[var(--primary)]">
                                        {{ $item->siswa_count }}
                                    </span>
                                </td>
                                <td class="px-5 py-3.5">
    ...
    <div x-show="modalTambah"
        ...
        <div x-show="modalTambah"
             ...
             class="relative w-full max-w-md rounded-xl border border-[var(--border)] bg-[var(--card)] shadow-[var(--shadow-elevated)]">
            <div class="flex items-center justify-between border-b border-[var(--border)] px-5 py-4">
                <h2 class="font-display text-base font-semibold text-[var(--foreground)]">Tambah Kelas</h2>
                <button @click="modalTambah = false" type="button"
                        class="flex h-8 w-8 items-center justify-center rounded-md text-[var(--muted-foreground)] hover:bg-[var(--muted)]">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
            ...
            <form method="POST" action="{{ route('admin.kelas.store') }}">
                ...
                <div class="space-y-4 px-5 py-5">
                    <x-ui.input name="nama_kelas" label="Nama Kelas" placeholder="Contoh: Kelas 1A" :required="true" />
                    <div class="grid grid-cols-2 gap-4">
                        <x-ui.input name="tingkat" label="Tingkat" placeholder="Contoh: 1" :required="true" />
                        <x-ui.input name="tahun_ajaran" label="Tahun Ajaran" placeholder="2025/2026" :required="true" />
                    </div>
                    <x-ui.select name="wali_kelas_id" label="Wali Kelas"
